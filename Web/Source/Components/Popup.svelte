    ...
    <ScoopedFrame width="[50%]" height="auto">
        <div class="w-full h-full flex flex-col items-start m-2 gap-y-2">
            <div class="w-full h-4/5 flex flex-col justify-center gap-y-2">
                <h1 class="text-2xl hy-impact-font font-medium text-gray-800 drop-shadow-md">{ title ?? $i18nInstance.t("component.popup.default_title") }</h1>
            ...
            <div class="w-full h-1/5 inline-flex justify-center gap-x-6">
                <button data-role="button" data-highlight class="w-full" on:click={() => Root.Choose(true) }>{ confirm ?? $i18nInstance.t("component.popup.button_confirm") }</button>
            </div>
        </div>
    </ScoopedFrame>
</Modal>
