...
<script lang="ts">
    ...
    export let Root: ConfirmPopupControl = new ConfirmPopupControl();
    ...
    <ScoopedFrame width="[30%]" height="auto">
        <div class="w-full h-full flex flex-col items-start m-2 gap-y-2">
            ...
            <div class="w-full h-1/5 inline-flex justify-center gap-x-6">
                <button class="input w-full" on:click={() => Root.Confirm(false) }>{ no }</button>
                <button class="input w-full" on:click={() => Root.Confirm(true) }>{ yes }</button>
            </div>
        </div>
    </ScoopedFrame>
</Modal>
